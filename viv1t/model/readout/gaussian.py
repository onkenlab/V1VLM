...
class GaussianReadout(Readout):
    ...
    def init_grid_predictor(
        ...
        source_grid = source_grid - torch.mean(source_grid, dim=0, keepdim=True)
    ...
    def sample_grid(self, batch_size: int | torch.Tensor, sample: bool = None):
        ...
        if self.gaussian_type == "full":
            grid = einsum(self.sigma, norm, "a n c d, b n i d -> b n i c") + self.mu
        else:
...
@register("gaussian")
class GaussianReadouts(Readouts):
    def __init__(
        self,
        args: Any,
        input_shape: tuple[int, int, int, int],
        ...
        super(GaussianReadouts, self).__init__(
            args,
            input_shape=input_shape,
            neuron_coordinates=neuron_coordinates,
            mean_responses=mean_responses,
