...
class GaussianReadout(nn.Module):
    def __init__(
        ...
        self.num_neurons = len(neuron_coordinates)
    ...
    def sample_grid(self, batch_size: int | torch.Tensor, sample: bool = None):
        """
    ...
    def forward(
        self, inputs: torch.Tensor, shifts: torch.Tensor = None, sample: bool = None
    ):
        """
        ...
        if shifts is not None:
            shifts = rearrange(shifts, "b c t -> (b t) c")
...
class DepthGaussianReadout(Readout):
    def __init__(
        self,
        args: Any,
        input_shape: tuple[int, int, int, int],
        ...
        super(DepthGaussianReadout, self).__init__(
            args,
        ...
        # separate neurons by depth (z-value)
        depths = torch.unique(neuron_coordinates[:, -1], sorted=False).tolist()[::-1]
        groups = {
            depth: torch.where(neuron_coordinates[:, -1] == depth)[0]
            for depth in depths
        }
        # create one readout per depth value
        self.readouts = nn.ModuleDict(
            {
                str(int(depth)): GaussianReadout(
                    args,
                    input_shape=input_shape,
                    mouse_id=mouse_id,
                    neuron_coordinates=neuron_coordinates[groups[depth]],
                    mean_responses=mean_responses[groups[depth]],
                )
                for depth in depths
            }
        )
    ...
    def forward(
        ...
        outputs = torch.cat(
            [
                self.readouts[depth](inputs, shifts, sample, behaviors, pupil_centers)
                for depth in self.readouts.keys()
            ],
            dim=1,
        )
        return outputs
...
@register("depth-gaussian")
class DepthGaussianReadouts(Readouts):
    def __init__(
        self,
        args: Any,
        input_shape: tuple[int, int, int, int],
        ...
        super(DepthGaussianReadouts, self).__init__(
            args,
            readout=DepthGaussianReadout,
            input_shape=input_shape,
            neuron_coordinates=neuron_coordinates,
            mean_responses=mean_responses,
        )
