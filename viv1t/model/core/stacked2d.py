...
from viv1t.model.modules import AdaptiveELU
...
class Scale2DLayer(nn.Module):
    ...
    def __init__(self, channels, initial=1, **kwargs):
        ...
        self.scale = nn.Parameter(torch.empty((1, channels, 1, 1)).fill_(initial))
...
class Stacked2d(nn.Module):
    def __init__(
        ...
        stride: int = 1,
        input_stride: int = 1,
        final_nonlinearity: bool = False,
        elu_shift: Tuple[int, int] = (0, 0),
        bias: bool = True,
        pad_input: bool = False,
        hidden_padding: int = None,
        ...
        stack: Union[int, Iterable] = -1,
        use_avg_reg: bool = False,
        depth_separable: bool = False,
        attention_conv: bool = False,
        nonlinearity_config: Dict = None,
    ):
        super(Stacked2d, self).__init__()
        ...
        if independent_bn_bias:
            if not bias or not batch_norm_scale or not final_batchnorm_scale:
                warnings.warn(
                    "The default of independent_bn_bias=True will ignore the "
                    "kwargs bias, batch_norm_scale, and final_batchnorm_scale "
                    "when initializing the batchnorm. If you want to use these "
                    "arguments, please set independent_bn_bias=False."
                )
        ...
        self.num_layers = args.core_cnn_layers
        self.register_buffer("gamma_input", torch.tensor(args.core_cnn_input_reg))
        self.register_buffer("gamma_hidden", torch.tensor(args.core_cnn_hidden_reg))

        hidden_channels = args.core_cnn_filters
        ...
        self.input_kern = args.core_cnn_input_kernel
        self.hidden_kern = args.core_cnn_hidden_kernel
        ...
        self.momentum = args.core_momentum
        ...
        if self.ignore_group_sparsity and self.gamma_hidden > 0:
            warnings.warn(
                ...
                "type. Hidden channels will not be regularized and "
                "gamma_hidden is ignored."
            )
