...
class Factorized3d(nn.Module):
    def __init__(
        ...
        if hasattr(args, "core_linear") and args.core_linear:
            hidden_nonlinearities = "identity"
            batch_norm = False
            if args.verbose:
                print("Remove non-linearity in core module.")
...
class FactorizedCore(Core):
    def __init__(
        ...
        """
        super(FactorizedCore, self).__init__(
            args, input_shape=input_shape, verbose=verbose
        )
        self.input_shape = input_shape
        ...
        self.cnn = Factorized3d(args, input_shape=input_shape)
